...
class Cart extends Component {
    ...
    componentDidMount() {
        Promise.all([this.getCartData(), this.getAllCountries(), this.getUserAddresses()]);
    }
    ...
    render() {
        const { step, qty, selectedLocation, selectedPayment, fields, AddAddress, books, pageLoader, locations, countries } = this.state;
                                                ...
                                                <span
                                                    ...
                                                    onClick={() => {
                                                        if (selectedLocation.id) {
                                                            ...
                                                            displayAlert("Warning", "Please Select Location", "warning");
                                                        }
                                                    }}
                                                >
                                ...
                                <Col md={6}>
                                    {step === 1 ? (
                                        <>
                                            ...
                                            {books.map((item) => {
                                                return <HorizontalProduct withDelete data={item} reFetchData={() => this.getCartData()} />;
                                            })}
                                        </>
                                    ) : step === 2 ? (
                                        <>
                                            <h4>Location</h4>
                                            {locations.map((item) => {
                                                return <Location widthCheck locationData={item} getUserAddresses={() => this.getUserAddresses()} selectedLocation={selectedLocation} changeLocation={(locationData) => this.selectedLocation(locationData)} />;
                                            })}
                                            ...
                                            onClick={() => {
                                                if (step === 3) {
                                                    if (selectedPayment !== -1) {
                                                        ...
                                                        displayAlert("Warning", "Please Select Payment Method", "warning");
                                                    }
                                                } else if (step === 2) {
                                                    if (selectedLocation.id) {
                                                        this.setState({
    ...
    totalAmount = (books) => {
        const totalAmount = books?.reduce((acc, currentValue) => {
            ...
            if (currentValue?.book?.campaign) {
                price = this.newPrice(currentValue?.book?.campaign, currentValue?.book?.price);
            } else {
    ...
    checkout = async () => {
        ...
        const { success, data } = await generalServices.checkout({
