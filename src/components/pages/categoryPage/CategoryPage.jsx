...
class CategoryPage extends Component {
  ...
  getCategoryBooks = async () => {
    if (+this.props.match.params.catType === 1) {
      const { success, data } = await generalServices.getClass(
      ...
      const { success, data } = await generalServices.getSubject(
        this.props.match.params.id
      );
      if (!success) return;
  ...
  render() {
    const { pageLoader, books, relatedBooks, title } = this.state;
    return pageLoader ? (
      <MainLoader />
    ) : (
        ...
        <div className="favorites-products">
          <Container>
            <h3 className="page-title">
              {this.props.match.params.key ?? title}
            </h3>
                ...
                {books?.map((item, index) => {
                  return (
                    <Col key={index} sm={6} md={4} xl={3}>
                      <Product
                        item={item}
                        reFetchData={() => this.getCategoryBooks()}
                      />
                    </Col>
                  );
                })}
