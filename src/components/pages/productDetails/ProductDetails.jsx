...
class ProductDetails extends Component {
    ...
    render() {
        ...
        const { menuOpen, images, qty, descriptionText, book, relatedBooks, pageLoader, key, classes, subjects } = this.state;

        const readMoreText = "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book?. It has survived not only five centuries, but also the leap into electronic typesetting,";
        return pageLoader ? (
            <MainLoader />
        ) : (
            <div className="product-details-main">
                                    ...
                                    <div className="sideMenu_List">
                                        <ul className="mb-0 mt-3">
                                            ...
                                            {classes.map(({ name, id }) => {
                                                return (
                                                    <li>
                                                        <a href={`/CategoryPage/1/${id}`} onClick={closeMenu}>
                                                            {name}
                                                        </a>
                                                    </li>
                                                );
                                            ...
                                            {subjects.map(({ name, id }) => {
                                                return (
                                                    <li>
                                                        <a href={`/CategoryPage/2/${id}`} onClick={closeMenu}>
                                                            {name}
                                                        </a>
                                                    </li>
                                                );
                ...
                <Container>
                    ...
                    <div className="product-browser-detail">
                        <Row>
                            <Col sm={12} lg={6}>
                                {" "}
                                <ImageGallery items={images} thumbnailPosition="bottom" showNav={false} showFullscreenButton={false} showPlayButton={false} showThumbnails={true} renderItem={(images) => this._renderCustomControls(images)} renderThumbInner={(images) => this._renderCustomThumbs(images)} />
                            </Col>
                            <Col sm={7} lg={4}>
                                <div className="product-browser-detail__info">
                                    ...
                                    <p>
                                        ...
                                        {book?.description?.length > 300 &&
                                            (descriptionText ? (
                                                <span onClick={() => this.setState({ descriptionText: false })} className="read-more">
                                                    Show less
                                                </span>
                                            ) : (
                                                <span onClick={() => this.setState({ descriptionText: true })} className="read-more">
                                                    read more
                                                </span>
                                            ))}
                                    </p>
                                    ...
                                    <div className="product-browser-detail__info__buttons">
                                        ...
                                        <div className="add-to-cart-wish-list big-sale__card--buttons flex">
                                            <button className={book?.is_fav ? "add-to-fav--active add-to-fav flex" : "add-to-fav flex"} onClick={() => this.addRemoveFavorites(book?.id, book?.is_fav)}>
                ...
                <div className="product-details">
                    <NewTitle title="Related Book" data={relatedBooks} reFetchData={() => this.getBookData()} />
                    <AgentOf />
                </div>
            </div>
        );
    ...
    addRemoveFavorites = async (id, is_fav) => {
        ...
        if (!success) return;
        this.props.setUser({ fav_count: is_fav ? this.props.currentUser.fav_count - 1 : this.props.currentUser.fav_count + 1 });
